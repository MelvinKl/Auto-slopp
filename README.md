...
1. **Ralph-enabled mode**: The PR body is generated from the refined task file via the Ralph executor. The `_ensure_issue_link_in_pr_body` helper function guarantees that at least one valid closing keyword is present in the final PR body.

2. **Non-Ralph mode**: The default PR body is generated by `TaskSource.get_default_pr_body()`, which also uses the `_ensure_issue_link_in_pr_body` helper for defense-in-depth.
