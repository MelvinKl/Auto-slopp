...
- **No Error Messages in GitHub Issues**: The program never posts raw error messages on GitHub. Error details (tracebacks, CLI failures, etc.) are only written to local logs. A defensive guard in the posting layer (`looks_like_error_message()` in `src/auto_slopp/utils/github_operations.py`) is applied to every posting path — issue comments, PR comments, PR creation (title and body), and PR reviews — so no caller can bypass this guarantee.
...
The following GitHub closing keywords are supported (case-insensitive):
...
1. **Ralph-enabled mode**: The PR body is generated from the refined task file via the Ralph executor. The `ensure_issue_link_in_pr_body` helper function guarantees that at least one valid closing keyword is present in the final PR body.

2. **Non-Ralph mode**: The default PR body is generated by `TaskSource.get_default_pr_body()`, which also uses the `ensure_issue_link_in_pr_body` helper for defense-in-depth.

If the generated PR body already contains a valid closing keyword for the issue, it is preserved. If no valid closing keyword is found, `Closes #<issue-number>` is prepended to the PR body.

This ensures that every PR created by Auto-slopp is properly linked to its source issue, enabling automatic issue closure when the PR is merged.
