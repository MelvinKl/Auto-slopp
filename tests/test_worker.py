...
class TestWorkerBase:
    ...
    def test_worker_can_return_any_type(self):
        ...
        assert string_worker.run(repo_path) == "string result"

        none_worker = NoneWorker()
        assert none_worker.run(repo_path) is None

        list_worker = ListWorker()
        assert list_worker.run(repo_path) == [1, 2, 3]
    ...
    def test_worker_can_have_custom_init(self):
        ...
        result = worker.run(Path("/test"))

        assert result["name"] == "test_worker"
        assert result["status"] == "completed"

        disabled_worker = ConfigurableWorker("disabled_worker", enabled=False)
        result = disabled_worker.run(Path("/test"))

        assert result["status"] == "disabled"

    def test_worker_can_handle_path_parameters(self, temp_repo_dir):
    ...
    def test_worker_can_import_and_use_dependencies(self, temp_repo_dir):
