...
class TestRalphExecutorUnavailability:
    """Tests for RalphExecutor._is_llm_unavailable and UNAVAILABILITY_PATTERNS."""

    def _make_executor(self) -> RalphExecutor:
        """Create a minimal RalphExecutor for testing."""
        return RalphExecutor(
            logger=logging.getLogger("test"),
            agent_args=[],
            timeout=30,
            execute_fn=lambda *args, **kwargs: {"success": True},
            has_changes_fn=lambda path: False,
            commit_fn=lambda path, msg, push: (True, None),
            max_iterations=10,
        )

    def test_is_llm_unavailable_rate_limit(self):
        """Test that rate limit errors are detected as transient."""
        executor = self._make_executor()
        assert executor._is_llm_unavailable("429 Too Many Requests") is True
        assert executor._is_llm_unavailable("rate limit exceeded") is True
        assert executor._is_llm_unavailable("Rate limit hit") is True

    def test_is_llm_unavailable_timeout(self):
        """Test that timeout errors are detected as transient."""
        executor = self._make_executor()
        assert executor._is_llm_unavailable("408 Request Timeout") is True
        assert executor._is_llm_unavailable("socket timeout") is True
        assert executor._is_llm_unavailable("connection timeout") is True

    def test_is_llm_unavailable_connection_errors(self):
        """Test that connection errors are detected as transient."""
        executor = self._make_executor()
        assert executor._is_llm_unavailable("connection reset") is True
        assert executor._is_llm_unavailable("connection refused") is True
        assert executor._is_llm_unavailable("ECONNRESET") is True
        assert executor._is_llm_unavailable("ETIMEDOUT") is True
        assert executor._is_llm_unavailable("broken pipe") is True

    def test_is_llm_unavailable_502_503_504(self):
        """Test that 502/503/504 errors are detected as transient."""
        executor = self._make_executor()
        assert executor._is_llm_unavailable("502 Bad Gateway") is True
        assert executor._is_llm_unavailable("503 Service Unavailable") is True
        assert executor._is_llm_unavailable("504 Gateway Timeout") is True
        assert executor._is_llm_unavailable("service unavailable") is True

    def test_is_llm_unavailable_not_500(self):
        """Test that 500 errors are NOT detected as transient (persistent bug)."""
        executor = self._make_executor()
        assert executor._is_llm_unavailable("500 Internal Server Error") is False
        assert executor._is_llm_unavailable("HTTP 500") is False

    def test_is_llm_unavailable_generic_error(self):
        """Test that generic errors are not detected as transient."""
        executor = self._make_executor()
        assert executor._is_llm_unavailable("something went wrong") is False
        assert executor._is_llm_unavailable("invalid API key") is False
        assert executor._is_llm_unavailable("") is False

    def test_unavailability_patterns_is_tuple(self):
        """Test that UNAVAILABILITY_PATTERNS is a tuple for immutability."""
        assert isinstance(RalphExecutor.UNAVAILABILITY_PATTERNS, tuple)

    def test_unavailability_patterns_excludes_500(self):
        """Test that 500 Internal Server Error is not in the patterns."""
        patterns = RalphExecutor.UNAVAILABILITY_PATTERNS
        for pattern in patterns:
            assert "500" not in pattern.lower()
            assert "internal server error" not in pattern.lower()
