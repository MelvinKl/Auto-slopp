                ...
                if not ralph_result.get("success", False):
                    ...
                    if ralph_result.get("max_loops_reached", False):
                        ...
                        if skip_reason:
                            ...
                            # The generic "Ralph loop failed: ..." message in
                            # ralph_error does not describe the actual failure;
                            # carry the underlying mid-loop failure in
                            # result["root_error"] (and result["error"]) so the
                            # two don't contradict each other.
                            result["root_error"] = skip_reason
                            result["error"] = skip_reason
                            return self._skip_task(result, task, skip_reason)
