...
Vikunja, following the same patterns used by IssueWorker.
"""
...
class VikunjaTaskSource(TaskSource):
    ...
    def _project_identifier(repo_path: Path) -> str:
        """Build a stable 10-character project identifier for a repository path.

        Args:
            repo_path: Path to the repository directory

        Returns:
            Project identifier of exactly 10 characters (Vikunja requirement)
        """
    ...
    def get_tasks(self, repo_path: Path) -> list[Task]:
        ...
        """
        project_name = repo_path.name
        project_identifier = self._project_identifier(repo_path)
    ...
    def get_default_pr_body(self, task: Task) -> str:
        ...
        """
        body = (
            f"Vikunja Task #{task.id}: {task.title}\n\n{task.body}\n\n"
            "*This pull request was automatically generated by the IssueWorker.*"
        )
