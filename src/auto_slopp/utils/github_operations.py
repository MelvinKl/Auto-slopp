...
def _sanitize_for_posting(text: str, generic_text: str, field: str, repo_name: str) -> str:
    """Return ``text`` unless it looks like a raw error message.

    Part of the guarantee that raw error details are never posted to GitHub:
    when ``text`` looks like an error message, ``generic_text`` is substituted
    and the substitution is only logged locally.

    Args:
        text: Text intended for a GitHub-facing field (title, body, ...)
        generic_text: Safe placeholder returned when ``text`` is error-like
        field: Name of the field, used in log messages only
        repo_name: Repository name, used in log messages only

    Returns:
        ``text`` unchanged when safe, otherwise ``generic_text``
    """
    if looks_like_error_message(text):
        logger.warning(
            f"Refusing to post error-like {field} in {repo_name}; "
            f"raw error details are never posted to GitHub. Substituting generic placeholder. "
            f"Original {field}: {text!r}"
        )
        return generic_text
    return text
...
def create_pull_request(
    ...
    Returns:
        ...
        Never posts error-like text: if the title or body looks like a raw
        error message, it is substituted with a safe generic value and the
        substitution is only logged locally.
    """
    safe_title = _sanitize_for_posting(title, "Task update", "PR title", repo_dir.name)
    safe_body = _sanitize_for_posting(body, "Generated by Auto-slopp.", "PR body", repo_dir.name)
    try:
        result = _run_gh_command(
            repo_dir,
            "pr",
...
def comment_on_pr(repo_dir: Path, pr_number: int, comment: str) -> bool:
    ...
    if looks_like_error_message(comment):
        logger.warning(
            f"Refusing to post error-like message to PR #{pr_number} in {repo_dir.name}; "
            f"raw error details are never posted to GitHub. Message: {comment!r}"
        )
        return False
    try:
        result = _run_gh_command(
...
def submit_pr_review(repo_dir: Path, pr_number: int, body: str, event: str = "COMMENT") -> bool:
    ...
    Returns:
        True if successful, False otherwise. Never posts error-like text:
        if the body looks like a raw error message, it is substituted with
        a safe generic value and the substitution is only logged locally.
    """
    safe_body = _sanitize_for_posting(
        body, "Auto-slopp review: see local logs for details.", "review body", repo_dir.name
    )
    try:
