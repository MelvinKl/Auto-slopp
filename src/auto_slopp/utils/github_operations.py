...
def create_pull_request(
    ...
    """
    safe_title = title
    if looks_like_error_message(title):
        logger.warning(
            f"Refusing to post error-like PR title in {repo_dir.name}; "
            f"raw error details are never posted to GitHub. Substituting generic title. "
            f"Original title: {title!r}"
        )
        safe_title = "Task update"
    safe_body = body
    if looks_like_error_message(body):
        logger.warning(
            f"Refusing to post error-like PR body in {repo_dir.name}; "
            f"raw error details are never posted to GitHub. Substituting generic body. "
            f"Original body: {body!r}"
        )
        safe_body = "Generated by Auto-slopp."
    try:
        result = _run_gh_command(
            repo_dir,
            "pr",
...
def submit_pr_review(repo_dir: Path, pr_number: int, body: str, event: str = "COMMENT") -> bool:
    ...
    if looks_like_error_message(body):
        logger.warning(
            f"Refusing to post error-like review body for PR #{pr_number} in {repo_dir.name}; "
            f"raw error details are never posted to GitHub. Substituting generic body. "
            f"Original body: {body!r}"
        )
        safe_body = "Auto-slopp review: see local logs for details."
    try:
