...
class MembershipFormModelEmbeddedValidationError(BaseModel):
    """
    MembershipFormModelEmbeddedValidationError
    """ # noqa: E501
    base: Optional[ErrorResponse] = None
    principal: Optional[ErrorResponse] = None
    roles: Optional[ErrorResponse] = None
    __properties: ClassVar[List[str]] = ["base", "principal", "roles"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "base": ErrorResponse.from_dict(obj["base"]) if obj.get("base") is not None else None,
            "principal": ErrorResponse.from_dict(obj["principal"]) if obj.get("principal") is not None else None,
            "roles": ErrorResponse.from_dict(obj["roles"]) if obj.get("roles") is not None else None
        })
        return _obj
