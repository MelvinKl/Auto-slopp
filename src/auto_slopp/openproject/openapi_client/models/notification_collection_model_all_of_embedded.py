...
class NotificationCollectionModelAllOfEmbedded(BaseModel):
    """
    NotificationCollectionModelAllOfEmbedded
    """ # noqa: E501
    elements: List[NotificationModel]
    details_schemas: List[SchemaModel] = Field(alias="detailsSchemas")
    __properties: ClassVar[List[str]] = ["elements", "detailsSchemas"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in details_schemas (list)
        _items = []
        if self.details_schemas:
            for _item_details_schemas in self.details_schemas:
                if _item_details_schemas:
                    _items.append(_item_details_schemas.to_dict())
            _dict['detailsSchemas'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "elements": [NotificationModel.from_dict(_item) for _item in obj["elements"]] if obj.get("elements") is not None else None,
            "detailsSchemas": [SchemaModel.from_dict(_item) for _item in obj["detailsSchemas"]] if obj.get("detailsSchemas") is not None else None
        })
        return _obj
