...
class ProjectPhaseModel(BaseModel):
    ...
    id: Annotated[int, Field(strict=True, ge=1)] = Field(description="The project phase's id")
    name: StrictStr
    active: StrictBool
    created_at: datetime = Field(description="Time of creation", alias="createdAt")
    updated_at: datetime = Field(description="Time of the most recent change to the project phase", alias="updatedAt")
    links: Optional[ProjectPhaseModelLinks] = Field(default=None, alias="_links")
    __properties: ClassVar[List[str]] = ["_type", "id", "name", "active", "createdAt", "updatedAt", "_links"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ProjectPhase']):
            raise ValueError("must be one of enum values ('ProjectPhase')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "_links": ProjectPhaseModelLinks.from_dict(obj["_links"]) if obj.get("_links") is not None else None
        })
        return _obj
