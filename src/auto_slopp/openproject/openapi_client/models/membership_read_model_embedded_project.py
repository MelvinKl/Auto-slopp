...
MEMBERSHIPREADMODELEMBEDDEDPROJECT_ONE_OF_SCHEMAS = ["PortfolioModel", "ProgramModel", "ProjectModel"]

class MembershipReadModelEmbeddedProject(BaseModel):
    """
    MembershipReadModelEmbeddedProject
    """
    # data type: ProjectModel
    oneof_schema_1_validator: Optional[ProjectModel] = None
    # data type: ProgramModel
    oneof_schema_2_validator: Optional[ProgramModel] = None
    # data type: PortfolioModel
    oneof_schema_3_validator: Optional[PortfolioModel] = None
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = MembershipReadModelEmbeddedProject.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in MembershipReadModelEmbeddedProject with oneOf schemas: PortfolioModel, ProgramModel, ProjectModel. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in MembershipReadModelEmbeddedProject with oneOf schemas: PortfolioModel, ProgramModel, ProjectModel. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into MembershipReadModelEmbeddedProject with oneOf schemas: PortfolioModel, ProgramModel, ProjectModel. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into MembershipReadModelEmbeddedProject with oneOf schemas: PortfolioModel, ProgramModel, ProjectModel. Details: " + ", ".join(error_messages))
        else:
            return instance
