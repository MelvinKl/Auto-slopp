...
class StorageReadModel(BaseModel):
    ...
    StorageReadModel
    """ # noqa: E501
    id: StrictInt = Field(description="Storage id")
    type: StrictStr = Field(alias="_type")
    name: StrictStr = Field(description="Storage name")
    storage_audience: Optional[StrictStr] = Field(default=None, description="The audience that the storage expects in tokens for requests to it, usually the storage's client ID at the identity provider.  This is only required for authentication through single-sign-on and so far only supported for provider type Nextcloud.", alias="storageAudience")
    token_exchange_scope: Optional[StrictStr] = Field(default=None, description="The scope that will be requested when requesting a token for the storage through token exchange. Has no effect if no token exchange is performed.  This is only required for authentication through single-sign-on and so far only supported for provider type Nextcloud.", alias="tokenExchangeScope")
    tenant_id: Optional[StrictStr] = Field(default=None, description="The tenant id of a file storage of type OneDrive.  Ignored if the provider type is not OneDrive. May be null if the storage is not configured completely.", alias="tenantId")
    drive_id: Optional[StrictStr] = Field(default=None, description="The drive id of a file storage of type OneDrive.  Ignored if the provider type is not OneDrive. May be null if the storage is not configured completely.", alias="driveId")
    has_application_password: Optional[StrictBool] = Field(default=None, description="Whether the storage has the application password to use for the Nextcloud storage.  Ignored if the provider type is not Nextcloud.", alias="hasApplicationPassword")
    forbidden_file_name_characters: Optional[StrictStr] = Field(default=None, description="A string with all the characters forbidden to be used for file and folder names in the storage. Used by OpenProject to avoid creating files with unsupported names (e.g. when creating project folders).  Only supported for provider type Nextcloud so far.", alias="forbiddenFileNameCharacters")
    created_at: Optional[datetime] = Field(default=None, description="Time of creation", alias="createdAt")
    updated_at: Optional[datetime] = Field(default=None, description="Time of the most recent change to the storage", alias="updatedAt")
    configured: Optional[StrictBool] = Field(default=None, description="Indication, if the storage is fully configured")
    embedded: Optional[StorageReadModelEmbedded] = Field(default=None, alias="_embedded")
    links: StorageReadModelLinks = Field(alias="_links")
    __properties: ClassVar[List[str]] = ["id", "_type", "name", "storageAudience", "tokenExchangeScope", "tenantId", "driveId", "hasApplicationPassword", "forbiddenFileNameCharacters", "createdAt", "updatedAt", "configured", "_embedded", "_links"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Storage']):
            raise ValueError("must be one of enum values ('Storage')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "_type": obj.get("_type"),
            "name": obj.get("name"),
            "storageAudience": obj.get("storageAudience"),
            "tokenExchangeScope": obj.get("tokenExchangeScope"),
            "tenantId": obj.get("tenantId"),
            "driveId": obj.get("driveId"),
            "hasApplicationPassword": obj.get("hasApplicationPassword"),
            "forbiddenFileNameCharacters": obj.get("forbiddenFileNameCharacters"),
            "createdAt": obj.get("createdAt"),
            "updatedAt": obj.get("updatedAt"),
            "configured": obj.get("configured"),
            "_embedded": StorageReadModelEmbedded.from_dict(obj["_embedded"]) if obj.get("_embedded") is not None else None,
            "_links": StorageReadModelLinks.from_dict(obj["_links"]) if obj.get("_links") is not None else None
        })
        return _obj
