...
class StorageFileUploadPreparationModel(BaseModel):
    """
    StorageFileUploadPreparationModel
    """ # noqa: E501
    project_id: Annotated[int, Field(strict=True, ge=1)] = Field(description="The project identifier, from where a user starts uploading a file.", alias="projectId")
    file_name: StrictStr = Field(description="The file name.", alias="fileName")
    parent: StrictStr = Field(description="The directory to which the file is to be uploaded. For root directories, the value `/` must be provided.")
    __properties: ClassVar[List[str]] = ["projectId", "fileName", "parent"]
