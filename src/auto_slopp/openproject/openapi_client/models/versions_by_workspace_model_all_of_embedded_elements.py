...
class VersionsByWorkspaceModelAllOfEmbeddedElements(BaseModel):
    """
    Collection of Versions
    """ # noqa: E501
    id: Annotated[int, Field(strict=True, ge=1)] = Field(description="Version id")
    type: StrictStr = Field(alias="_type")
    name: StrictStr = Field(description="Version name")
    description: Formattable
    start_date: date = Field(alias="startDate")
    end_date: date = Field(alias="endDate")
    status: StrictStr = Field(description="The current status of the version. This could be:  - *open*: if the version is available to be assigned to work packages in all shared projects - *locked*: if the version is not finished, but locked for further assignments to work packages - *closed*: if the version is finished")
    sharing: StrictStr = Field(description="The indicator of how the version is shared between projects. This could be:  - *none*: if the version is only available in the defining project - *descendants*: if the version is shared with the descendants of the defining project - *hierarchy*: if the version is shared with the descendants and the ancestors of the defining project - *tree*: if the version is shared with the root project of the defining project and all descendants of the root project - *system*: if the version is shared globally")
    created_at: datetime = Field(description="Time of creation", alias="createdAt")
    updated_at: datetime = Field(description="Time of the most recent change to the version", alias="updatedAt")
    links: Optional[VersionReadModelAllOfLinks] = Field(default=None, alias="_links")
    __properties: ClassVar[List[str]] = ["id", "_type", "name", "description", "startDate", "endDate", "status", "sharing", "createdAt", "updatedAt", "_links"]
    ...
    def sharing_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['none', 'descendants', 'hierarchy', 'tree', 'system']):
            raise ValueError("must be one of enum values ('none', 'descendants', 'hierarchy', 'tree', 'system')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "_links": VersionReadModelAllOfLinks.from_dict(obj["_links"]) if obj.get("_links") is not None else None
        })
        return _obj
