...
class MembershipReadModel(BaseModel):
    """
    MembershipReadModel
    """ # noqa: E501
    type: StrictStr = Field(alias="_type")
    id: Annotated[int, Field(strict=True, ge=1)] = Field(description="The membership's id")
    created_at: datetime = Field(description="The time the membership was created.", alias="createdAt")
    updated_at: datetime = Field(description="The time the membership was last updated.", alias="updatedAt")
    embedded: Optional[MembershipReadModelEmbedded] = Field(default=None, alias="_embedded")
    links: MembershipReadModelLinks = Field(alias="_links")
    __properties: ClassVar[List[str]] = ["_type", "id", "createdAt", "updatedAt", "_embedded", "_links"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Membership']):
            raise ValueError("must be one of enum values ('Membership')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "_embedded": MembershipReadModelEmbedded.from_dict(obj["_embedded"]) if obj.get("_embedded") is not None else None,
            "_links": MembershipReadModelLinks.from_dict(obj["_links"]) if obj.get("_links") is not None else None
        })
        return _obj
