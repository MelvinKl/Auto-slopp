...
class AttachmentsModelAllOfEmbeddedElements(BaseModel):
    """
    Collection of Attachments
    """ # noqa: E501
    id: Optional[Annotated[int, Field(strict=True, ge=1)]] = Field(default=None, description="Attachment's id")
    file_name: StrictStr = Field(description="The name of the uploaded file", alias="fileName")
    file_size: Optional[Annotated[int, Field(strict=True, ge=0)]] = Field(default=None, description="The size of the uploaded file in Bytes", alias="fileSize")
    description: Formattable = Field(description="A user provided description of the file")
    status: StrictStr
    content_type: StrictStr = Field(description="The files MIME-Type as determined by the server", alias="contentType")
    digest: AttachmentModelDigest
    created_at: datetime = Field(description="Time of creation", alias="createdAt")
    links: Optional[AttachmentModelLinks] = Field(default=None, alias="_links")
    __properties: ClassVar[List[str]] = ["id", "fileName", "fileSize", "description", "status", "contentType", "digest", "createdAt", "_links"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['uploaded', 'prepared', 'scanned', 'quarantined', 'rescan']):
            raise ValueError("must be one of enum values ('uploaded', 'prepared', 'scanned', 'quarantined', 'rescan')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "digest": AttachmentModelDigest.from_dict(obj["digest"]) if obj.get("digest") is not None else None,
            "createdAt": obj.get("createdAt"),
            "_links": AttachmentModelLinks.from_dict(obj["_links"]) if obj.get("_links") is not None else None
        })
        return _obj
