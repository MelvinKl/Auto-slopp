...
class NonWorkingDayModel(BaseModel):
    """
    NonWorkingDayModel
    """ # noqa: E501
    type: StrictStr = Field(alias="_type")
    var_date: date = Field(description="Date of the non-working day.", alias="date")
    name: StrictStr = Field(description="Descriptive name for the non-working day.")
    links: Optional[NonWorkingDayModelLinks] = Field(default=None, alias="_links")
    __properties: ClassVar[List[str]] = ["_type", "date", "name", "_links"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['NonWorkingDay']):
            raise ValueError("must be one of enum values ('NonWorkingDay')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "date": obj.get("date"),
            "name": obj.get("name"),
            "_links": NonWorkingDayModelLinks.from_dict(obj["_links"]) if obj.get("_links") is not None else None
        })
        return _obj
