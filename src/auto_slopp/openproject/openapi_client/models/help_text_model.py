...
class HelpTextModel(BaseModel):
    """
    HelpTextModel
    """ # noqa: E501
    type: StrictStr = Field(alias="_type")
    id: Annotated[int, Field(strict=True, ge=1)]
    attribute: StrictStr = Field(description="The attribute the help text is assigned to.")
    scope: StrictStr
    help_text: Formattable = Field(alias="helpText")
    links: HelpTextModelLinks = Field(alias="_links")
    __properties: ClassVar[List[str]] = ["_type", "id", "attribute", "scope", "helpText", "_links"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['HelpText']):
            raise ValueError("must be one of enum values ('HelpText')")
        return value

    @field_validator('scope')
    def scope_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['WorkPackage', 'Project']):
            raise ValueError("must be one of enum values ('WorkPackage', 'Project')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of help_text
        if self.help_text:
            _dict['helpText'] = self.help_text.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "scope": obj.get("scope"),
            "helpText": Formattable.from_dict(obj["helpText"]) if obj.get("helpText") is not None else None,
            "_links": HelpTextModelLinks.from_dict(obj["_links"]) if obj.get("_links") is not None else None
        })
        return _obj
