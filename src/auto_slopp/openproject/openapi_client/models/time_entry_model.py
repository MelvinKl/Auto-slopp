...
class TimeEntryModel(BaseModel):
    ...
    TimeEntryModel
    """ # noqa: E501
    id: Optional[Annotated[int, Field(strict=True, ge=1)]] = Field(default=None, description="The id of the time entry")
    comment: Optional[Formattable] = Field(default=None, description="A comment to the time entry")
    spent_on: Optional[date] = Field(default=None, description="The date the expenditure is booked for", alias="spentOn")
    hours: Optional[StrictStr] = Field(default=None, description="The time quantifying the expenditure")
    ongoing: Optional[StrictBool] = Field(default=None, description="Whether the time entry is actively tracking time")
    created_at: Optional[datetime] = Field(default=None, description="The time the time entry was created", alias="createdAt")
    start_time: Optional[datetime] = Field(default=None, description="The time the time entry was started, or null if the time entry does not have a start time.  The time is returned as UTC time, if presented to the user it should be converted to the user's timezone.  This field is only available if the global `allow_tracking_start_and_end_times` setting is enabled.", alias="startTime")
    end_time: Optional[datetime] = Field(default=None, description="The time the time entry was ended, or null if the time entry does not have a start time.  The time is returned as UTC time, if presented to the user it should be converted to the user's timezone.  This field is only available if the global `allow_tracking_start_and_end_times` setting is enabled.", alias="endTime")
    updated_at: Optional[datetime] = Field(default=None, description="The time the time entry was last updated", alias="updatedAt")
    links: Optional[TimeEntryModelAllOfLinks] = Field(default=None, alias="_links")
    __properties: ClassVar[List[str]] = ["id", "comment", "spentOn", "hours", "ongoing", "createdAt", "startTime", "endTime", "updatedAt", "_links"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "comment": Formattable.from_dict(obj["comment"]) if obj.get("comment") is not None else None,
            "spentOn": obj.get("spentOn"),
            "hours": obj.get("hours"),
            ...
            "_links": TimeEntryModelAllOfLinks.from_dict(obj["_links"]) if obj.get("_links") is not None else None
        })
        return _obj
