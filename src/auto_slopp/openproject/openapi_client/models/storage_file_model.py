...
class StorageFileModel(BaseModel):
    """
    StorageFileModel
    """ # noqa: E501
    id: StrictStr = Field(description="Linked file's id on the origin")
    name: StrictStr = Field(description="Linked file's name on the origin")
    mime_type: Optional[Annotated[str, Field(min_length=0, strict=True)]] = Field(default=None, description="MIME type of the linked file.  To link a folder entity, the custom MIME type `application/x-op-directory` MUST be provided. Otherwise it defaults back to an unknown MIME type.", alias="mimeType")
    size: Optional[Annotated[int, Field(strict=True, ge=0)]] = Field(default=None, description="file size on origin in bytes")
    created_at: Optional[datetime] = Field(default=None, description="Timestamp of the creation datetime of the file on the origin", alias="createdAt")
    last_modified_at: Optional[datetime] = Field(default=None, description="Timestamp of the datetime of the last modification of the file on the origin", alias="lastModifiedAt")
    created_by_name: Optional[StrictStr] = Field(default=None, description="Display name of the author that created the file on the origin", alias="createdByName")
    last_modified_by_name: Optional[StrictStr] = Field(default=None, description="Display name of the author that modified the file on the origin last", alias="lastModifiedByName")
    type: StrictStr = Field(alias="_type")
    location: StrictStr = Field(description="Location identification for file in storage")
    links: StorageFileModelAllOfLinks = Field(alias="_links")
    __properties: ClassVar[List[str]] = ["id", "name", "mimeType", "size", "createdAt", "lastModifiedAt", "createdByName", "lastModifiedByName", "_type", "location", "_links"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['StorageFile']):
            raise ValueError("must be one of enum values ('StorageFile')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "lastModifiedByName": obj.get("lastModifiedByName"),
            "_type": obj.get("_type"),
            "location": obj.get("location"),
            "_links": StorageFileModelAllOfLinks.from_dict(obj["_links"]) if obj.get("_links") is not None else None
        })
        return _obj
