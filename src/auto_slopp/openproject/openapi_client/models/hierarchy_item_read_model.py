...
class HierarchyItemReadModel(BaseModel):
    """
    HierarchyItemReadModel
    """ # noqa: E501
    type: StrictStr = Field(alias="_type")
    id: StrictInt = Field(description="Hierarchy item identifier")
    label: Optional[StrictStr] = Field(description="The label of the hierarchy item")
    short: Optional[StrictStr] = Field(description="The short name of the hierarchy item. If this attribute is set, the `weight` and the `formattedWeight` are `null`.")
    weight: Optional[StrictStr] = Field(description="The accurate weight of the hierarchy item. As a decimal precision number it is written as a string to not loose precision with conversion to a floating point number. If this attribute is set, the `short` is null.")
    formatted_weight: Optional[StrictStr] = Field(description="The formatted weight of the hierarchy item. The standard formatting of the OpenProject server is used to convert this number into a representable format - i.e. falling back to scientific notation for very small and very big numbers. If this attribute is set, the `short` is null.", alias="formattedWeight")
    depth: StrictInt = Field(description="The hierarchy depth. The root item has a depth of 0.")
    links: HierarchyItemReadModelLinks = Field(alias="_links")
    __properties: ClassVar[List[str]] = ["_type", "id", "label", "short", "weight", "formattedWeight", "depth", "_links"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['HierarchyItem']):
            raise ValueError("must be one of enum values ('HierarchyItem')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # set to None if short (nullable) is None
        # and model_fields_set contains the field
        if self.short is None and "short" in self.model_fields_set:
            _dict['short'] = None
        ...
        # set to None if formatted_weight (nullable) is None
        # and model_fields_set contains the field
        if self.formatted_weight is None and "formatted_weight" in self.model_fields_set:
            _dict['formattedWeight'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "formattedWeight": obj.get("formattedWeight"),
            "depth": obj.get("depth"),
            "_links": HierarchyItemReadModelLinks.from_dict(obj["_links"]) if obj.get("_links") is not None else None
        })
        return _obj
