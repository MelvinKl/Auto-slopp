...
class PrincipalModel(BaseModel):
    ...
    links: PrincipalModelLinks = Field(alias="_links")
    __properties: ClassVar[List[str]] = ["_type", "id", "name", "createdAt", "updatedAt", "_links"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['User', 'Group', 'PlaceholderUser']):
            raise ValueError("must be one of enum values ('User', 'Group', 'PlaceholderUser')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "_links": PrincipalModelLinks.from_dict(obj["_links"]) if obj.get("_links") is not None else None
        })
        return _obj
