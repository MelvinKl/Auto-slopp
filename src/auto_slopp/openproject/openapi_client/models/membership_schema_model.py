...
class MembershipSchemaModel(BaseModel):
    """
    MembershipSchemaModel
    """ # noqa: E501
    type: StrictStr = Field(alias="_type")
    dependencies: Optional[List[StrictStr]] = Field(default=None, description="A list of dependencies between one property's value and another property", alias="_dependencies")
    links: SchemaModelLinks = Field(alias="_links")
    id: SchemaPropertyModel
    created_at: SchemaPropertyModel = Field(alias="createdAt")
    updated_at: SchemaPropertyModel = Field(alias="updatedAt")
    notification_message: SchemaPropertyModel = Field(alias="notificationMessage")
    project: SchemaPropertyModel
    principal: SchemaPropertyModel
    roles: SchemaPropertyModel
    __properties: ClassVar[List[str]] = ["_type", "_dependencies", "_links", "id", "createdAt", "updatedAt", "notificationMessage", "project", "principal", "roles"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "_links": SchemaModelLinks.from_dict(obj["_links"]) if obj.get("_links") is not None else None,
            ...
            "notificationMessage": SchemaPropertyModel.from_dict(obj["notificationMessage"]) if obj.get("notificationMessage") is not None else None,
            "project": SchemaPropertyModel.from_dict(obj["project"]) if obj.get("project") is not None else None,
            "principal": SchemaPropertyModel.from_dict(obj["principal"]) if obj.get("principal") is not None else None,
            "roles": SchemaPropertyModel.from_dict(obj["roles"]) if obj.get("roles") is not None else None
        })
        return _obj
