...
WORKSPACECOLLECTIONMODELALLOFEMBEDDEDELEMENTS_ONE_OF_SCHEMAS = ["PortfolioModel", "ProgramModel", "ProjectModel"]

class WorkspaceCollectionModelAllOfEmbeddedElements(BaseModel):
    """
    WorkspaceCollectionModelAllOfEmbeddedElements
    """
    # data type: PortfolioModel
    oneof_schema_1_validator: Optional[PortfolioModel] = None
    # data type: ProgramModel
    oneof_schema_2_validator: Optional[ProgramModel] = None
    # data type: ProjectModel
    oneof_schema_3_validator: Optional[ProjectModel] = None
    actual_instance: Optional[Union[PortfolioModel, ProgramModel, ProjectModel]] = None
    one_of_schemas: Set[str] = { "PortfolioModel", "ProgramModel", "ProjectModel" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = WorkspaceCollectionModelAllOfEmbeddedElements.model_construct()
        error_messages = []
        match = 0
        # validate data type: PortfolioModel
        if not isinstance(v, PortfolioModel):
            error_messages.append(f"Error! Input type `{type(v)}` is not `PortfolioModel`")
        else:
            match += 1
        # validate data type: ProgramModel
        if not isinstance(v, ProgramModel):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ProgramModel`")
        else:
            match += 1
        # validate data type: ProjectModel
        if not isinstance(v, ProjectModel):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ProjectModel`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in WorkspaceCollectionModelAllOfEmbeddedElements with oneOf schemas: PortfolioModel, ProgramModel, ProjectModel. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in WorkspaceCollectionModelAllOfEmbeddedElements with oneOf schemas: PortfolioModel, ProgramModel, ProjectModel. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into PortfolioModel
        try:
            instance.actual_instance = PortfolioModel.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into ProgramModel
        try:
            instance.actual_instance = ProgramModel.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into ProjectModel
        try:
            instance.actual_instance = ProjectModel.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into WorkspaceCollectionModelAllOfEmbeddedElements with oneOf schemas: PortfolioModel, ProgramModel, ProjectModel. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into WorkspaceCollectionModelAllOfEmbeddedElements with oneOf schemas: PortfolioModel, ProgramModel, ProjectModel. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], PortfolioModel, ProgramModel, ProjectModel]]:
