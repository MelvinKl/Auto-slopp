...
class DayModel(BaseModel):
    ...
    var_date: date = Field(description="Date of the day.", alias="date")
    name: StrictStr = Field(description="Descriptive name for the day.")
    working: StrictBool = Field(description="`true` for a working day, `false` otherwise.")
    links: Optional[DayModelLinks] = Field(default=None, alias="_links")
    __properties: ClassVar[List[str]] = ["_type", "date", "name", "working", "_links"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "_links": DayModelLinks.from_dict(obj["_links"]) if obj.get("_links") is not None else None
        })
        return _obj
