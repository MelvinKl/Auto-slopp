...
class GridReadModel(BaseModel):
    """
    GridReadModel
    """ # noqa: E501
    type: StrictStr = Field(alias="_type")
    id: Annotated[int, Field(strict=True, ge=1)] = Field(description="Grid's id")
    row_count: Annotated[int, Field(strict=True, ge=1)] = Field(description="The number of rows the grid has", alias="rowCount")
    column_count: Annotated[int, Field(strict=True, ge=1)] = Field(description="The number of columns the grid has", alias="columnCount")
    widgets: List[GridWidgetModel] = Field(description="The set of `GridWidget`s selected for the grid.  # Conditions  - The widgets must not overlap.")
    created_at: Optional[datetime] = Field(default=None, description="The time the grid was created.", alias="createdAt")
    updated_at: Optional[datetime] = Field(default=None, description="The time the grid was last updated.", alias="updatedAt")
    links: GridReadModelLinks = Field(alias="_links")
    __properties: ClassVar[List[str]] = ["_type", "id", "rowCount", "columnCount", "widgets", "createdAt", "updatedAt", "_links"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Grid']):
            raise ValueError("must be one of enum values ('Grid')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "widgets": [GridWidgetModel.from_dict(_item) for _item in obj["widgets"]] if obj.get("widgets") is not None else None,
            "createdAt": obj.get("createdAt"),
            "updatedAt": obj.get("updatedAt"),
            "_links": GridReadModelLinks.from_dict(obj["_links"]) if obj.get("_links") is not None else None
        })
        return _obj
