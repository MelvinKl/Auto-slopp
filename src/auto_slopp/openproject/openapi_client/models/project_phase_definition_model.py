...
class ProjectPhaseDefinitionModel(BaseModel):
    """
    ProjectPhaseDefinitionModel
    """ # noqa: E501
    type: StrictStr = Field(alias="_type")
    id: Annotated[int, Field(strict=True, ge=1)] = Field(description="The project phase definition's id")
    name: StrictStr
    start_gate: StrictBool = Field(alias="startGate")
    start_gate_name: Optional[StrictStr] = Field(alias="startGateName")
    finish_gate: StrictBool = Field(alias="finishGate")
    finish_gate_name: Optional[StrictStr] = Field(alias="finishGateName")
    created_at: datetime = Field(description="Time of creation", alias="createdAt")
    updated_at: datetime = Field(description="Time of the most recent change to the project phase definition", alias="updatedAt")
    links: Optional[ProjectPhaseDefinitionModelLinks] = Field(default=None, alias="_links")
    __properties: ClassVar[List[str]] = ["_type", "id", "name", "startGate", "startGateName", "finishGate", "finishGateName", "createdAt", "updatedAt", "_links"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ProjectPhaseDefinition']):
            raise ValueError("must be one of enum values ('ProjectPhaseDefinition')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # set to None if start_gate_name (nullable) is None
        # and model_fields_set contains the field
        if self.start_gate_name is None and "start_gate_name" in self.model_fields_set:
            _dict['startGateName'] = None

        # set to None if finish_gate_name (nullable) is None
        # and model_fields_set contains the field
        if self.finish_gate_name is None and "finish_gate_name" in self.model_fields_set:
            _dict['finishGateName'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "id": obj.get("id"),
            "name": obj.get("name"),
            "startGate": obj.get("startGate"),
            "startGateName": obj.get("startGateName"),
            "finishGate": obj.get("finishGate"),
            "finishGateName": obj.get("finishGateName"),
            "createdAt": obj.get("createdAt"),
            "updatedAt": obj.get("updatedAt"),
            "_links": ProjectPhaseDefinitionModelLinks.from_dict(obj["_links"]) if obj.get("_links") is not None else None
        })
        return _obj
