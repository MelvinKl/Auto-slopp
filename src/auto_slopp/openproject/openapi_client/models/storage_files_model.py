...
class StorageFilesModel(BaseModel):
    """
    StorageFilesModel
    """ # noqa: E501
    type: StrictStr = Field(alias="_type")
    files: List[StorageFileModel] = Field(description="List of files provided by the selected storage.")
    parent: StorageFileModel = Field(description="File of the currently selected parent directory.")
    ancestors: List[StorageFileModel] = Field(description="List of ancestors of the parent directory. Can be empty, if parent directory was root directory.")
    links: StorageFileModelAllOfLinks = Field(alias="_links")
    __properties: ClassVar[List[str]] = ["_type", "files", "parent", "ancestors", "_links"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['StorageFiles']):
            raise ValueError("must be one of enum values ('StorageFiles')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "_type": obj.get("_type"),
            "files": [StorageFileModel.from_dict(_item) for _item in obj["files"]] if obj.get("files") is not None else None,
            "parent": StorageFileModel.from_dict(obj["parent"]) if obj.get("parent") is not None else None,
            "ancestors": [StorageFileModel.from_dict(_item) for _item in obj["ancestors"]] if obj.get("ancestors") is not None else None,
