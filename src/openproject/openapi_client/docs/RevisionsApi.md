# openproject_client.RevisionsApi
...
# **view_revision**
> RevisionModel view_revision(id)
...
with openproject_client.ApiClient(configuration) as api_client:
    ...
    api_instance = openproject_client.RevisionsApi(api_client)
    id = 1 # int | Revision id

    try:
        # View revision
        api_response = api_instance.view_revision(id)
        print("The response of RevisionsApi->view_revision:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling RevisionsApi->view_revision: %s\n" % e)
```
