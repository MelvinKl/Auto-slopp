...
class RelationReadModelEmbedded(BaseModel):
    """
    RelationReadModelEmbedded
    """ # noqa: E501
    var_from: Optional[WorkPackageModel] = Field(default=None, alias="from")
    to: Optional[WorkPackageModel] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "from": WorkPackageModel.from_dict(obj["from"]) if obj.get("from") is not None else None,
            "to": WorkPackageModel.from_dict(obj["to"]) if obj.get("to") is not None else None
        })
        return _obj
