...
class StorageReadModelLinks(BaseModel):
    """
    StorageReadModelLinks
    """ # noqa: E501
    var_self: Link = Field(description="This storage resource. Contains the user defined storage name as title.  **Resource**: Storage", alias="self")
    type: Link = Field(description="The urn of the storage type. Currently Nextcloud and OneDrive storages are supported.  - urn:openproject-org:api:v3:storages:Nextcloud - urn:openproject-org:api:v3:storages:OneDrive  **Resource**: N/A")
    authentication_method: Optional[Link] = Field(default=None, description="The urn of the authentication method. Currently only Nextcloud storages support this setting.  - urn:openproject-org:api:v3:storages:authenticationMethod:TwoWayOAuth2 (default) - urn:openproject-org:api:v3:storages:authenticationMethod:OAuth2SSO  **Resource**: N/A", alias="authenticationMethod")
    origin: Optional[Link] = Field(default=None, description="Web URI of the storage instance. This link is ignored, if the storage is hosted in a cloud and has no own URL, like file storages of type OneDrive.  **Resource**: N/A")
    open: Link = Field(description="URI of the file storage location, from where the user usually starts browsing files.  **Resource**: N/A")
    authorization_state: Link = Field(description="The urn of the storage connection state. Can be one of:  - urn:openproject-org:api:v3:storages:authorization:Connected - urn:openproject-org:api:v3:storages:authorization:FailedAuthorization - urn:openproject-org:api:v3:storages:authorization:Error  **Resource**: N/A", alias="authorizationState")
    authorize: Optional[Link] = Field(default=None, description="The link to the starting point of the authorization cycle for a configured storage provider.  # Conditions  `authorizationState` is:  - urn:openproject-org:api:v3:storages:authorization:FailedAuthorization  **Resource**: N/A")
    oauth_application: Optional[Link] = Field(default=None, description="The OAuth 2 provider application linked to the storage.  # Conditions  - User has role `admin`  **Resource**: OAuthApplication", alias="oauthApplication")
    oauth_client_credentials: Optional[Link] = Field(default=None, description="The OAuth 2 credentials resource linked to the storage.  # Conditions  - User has role `admin`  **Resource**: OAuthClientCredentials", alias="oauthClientCredentials")
    __properties: ClassVar[List[str]] = ["self", "type", "authenticationMethod", "origin", "open", "authorizationState", "authorize", "oauthApplication", "oauthClientCredentials"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.authorization_state:
            _dict['authorizationState'] = self.authorization_state.to_dict()
        # override the default output from pydantic by calling `to_dict()` of authorize
        if self.authorize:
            _dict['authorize'] = self.authorize.to_dict()
        # override the default output from pydantic by calling `to_dict()` of oauth_application
        if self.oauth_application:
            _dict['oauthApplication'] = self.oauth_application.to_dict()
        # override the default output from pydantic by calling `to_dict()` of oauth_client_credentials
        if self.oauth_client_credentials:
            _dict['oauthClientCredentials'] = self.oauth_client_credentials.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "authenticationMethod": Link.from_dict(obj["authenticationMethod"]) if obj.get("authenticationMethod") is not None else None,
            "origin": Link.from_dict(obj["origin"]) if obj.get("origin") is not None else None,
            "open": Link.from_dict(obj["open"]) if obj.get("open") is not None else None,
            "authorizationState": Link.from_dict(obj["authorizationState"]) if obj.get("authorizationState") is not None else None,
            "authorize": Link.from_dict(obj["authorize"]) if obj.get("authorize") is not None else None,
            "oauthApplication": Link.from_dict(obj["oauthApplication"]) if obj.get("oauthApplication") is not None else None,
            "oauthClientCredentials": Link.from_dict(obj["oauthClientCredentials"]) if obj.get("oauthClientCredentials") is not None else None
        })
        return _obj
