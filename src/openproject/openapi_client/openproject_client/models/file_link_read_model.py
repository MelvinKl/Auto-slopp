...
class FileLinkReadModel(BaseModel):
    """
    FileLinkReadModel
    """ # noqa: E501
    id: Optional[StrictInt] = Field(default=None, description="File link id")
    ...
    updated_at: Optional[datetime] = Field(default=None, description="Time of the most recent change to the file link", alias="updatedAt")
    origin_data: Optional[FileLinkOriginDataModel] = Field(default=None, alias="originData")
    embedded: Optional[FileLinkReadModelEmbedded] = Field(default=None, alias="_embedded")
    links: Optional[FileLinkReadModelLinks] = Field(default=None, alias="_links")
    __properties: ClassVar[List[str]] = ["id", "_type", "createdAt", "updatedAt", "originData", "_embedded", "_links"]
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in set(['FileLink']):
            raise ValueError("must be one of enum values ('FileLink')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of origin_data
        if self.origin_data:
            _dict['originData'] = self.origin_data.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "originData": FileLinkOriginDataModel.from_dict(obj["originData"]) if obj.get("originData") is not None else None,
            "_embedded": FileLinkReadModelEmbedded.from_dict(obj["_embedded"]) if obj.get("_embedded") is not None else None,
            "_links": FileLinkReadModelLinks.from_dict(obj["_links"]) if obj.get("_links") is not None else None
        })
        return _obj
