...
class HelpTextModelLinks(BaseModel):
    """
    HelpTextModelLinks
    """ # noqa: E501
    var_self: Link = Field(description="This help text resource.  **Resource**: HelpText", alias="self")
    edit_text: Link = Field(description="Edit the help text entry.  **Resource**: text/html", alias="editText")
    attachments: Link = Field(description="The attachment collection of this help text.  **Resource**: AttachmentCollection")
    add_attachment: Link = Field(description="Add an attachment to the help text.", alias="addAttachment")
    __properties: ClassVar[List[str]] = ["self", "editText", "attachments", "addAttachment"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of edit_text
        if self.edit_text:
            _dict['editText'] = self.edit_text.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "editText": Link.from_dict(obj["editText"]) if obj.get("editText") is not None else None,
