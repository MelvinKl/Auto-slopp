...
class ReminderModelLinks(BaseModel):
    """
    ReminderModelLinks
    """ # noqa: E501
    var_self: Optional[Link] = Field(default=None, description="This reminder  **Resource**: Reminder", alias="self")
    creator: Link = Field(description="The person that created the reminder  **Resource**: User")
    remindable: Link = Field(description="The resource that the reminder is associated with  **Resource**: WorkPackage")
    __properties: ClassVar[List[str]] = ["self", "creator", "remindable"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of remindable
        if self.remindable:
            _dict['remindable'] = self.remindable.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "remindable": Link.from_dict(obj["remindable"]) if obj.get("remindable") is not None else None
        })
        return _obj
