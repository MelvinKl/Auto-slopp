...
class ToggleActivityEmojiReactionRequest(BaseModel):
    """
    ToggleActivityEmojiReactionRequest
    """ # noqa: E501
    reaction: StrictStr = Field(description="The emoji reaction identifier")
    __properties: ClassVar[List[str]] = ["reaction"]

    @field_validator('reaction')
    def reaction_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['thumbs_up', 'thumbs_down', 'grinning_face_with_smiling_eyes', 'confused_face', 'heart', 'party_popper', 'rocket', 'eyes']):
            raise ValueError("must be one of enum values ('thumbs_up', 'thumbs_down', 'grinning_face_with_smiling_eyes', 'confused_face', 'heart', 'party_popper', 'rocket', 'eyes')")
        return value
