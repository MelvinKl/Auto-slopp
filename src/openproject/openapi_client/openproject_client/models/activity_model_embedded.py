...
class ActivityModelEmbedded(BaseModel):
    """
    ActivityModelEmbedded
    """ # noqa: E501
    attachments: Optional[AttachmentsModel] = Field(default=None, description="Collection of attachments for this activity")
    work_package: Optional[WorkPackageModel] = Field(default=None, description="The work package this activity belongs to  # Conditions  Only embedded when the `journable` of the activity is a work package", alias="workPackage")
    emoji_reactions: Optional[EmojiReactionsModel] = Field(default=None, description="Collection of emoji reactions for this activity", alias="emojiReactions")
    __properties: ClassVar[List[str]] = ["attachments", "workPackage", "emojiReactions"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "attachments": AttachmentsModel.from_dict(obj["attachments"]) if obj.get("attachments") is not None else None,
            "workPackage": WorkPackageModel.from_dict(obj["workPackage"]) if obj.get("workPackage") is not None else None,
            "emojiReactions": EmojiReactionsModel.from_dict(obj["emojiReactions"]) if obj.get("emojiReactions") is not None else None
        })
        return _obj
