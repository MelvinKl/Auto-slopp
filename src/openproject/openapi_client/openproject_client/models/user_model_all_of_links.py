...
class UserModelAllOfLinks(BaseModel):
    """
    UserModelAllOfLinks
    """ # noqa: E501
    show_user: Optional[Link] = Field(default=None, description="A relative path to show the user in the web application.  # Condition  - User is not a new record - User is not `locked`", alias="showUser")
    update_immediately: Optional[Link] = Field(default=None, description="A link to update the user resource.  # Conditions  - `admin`", alias="updateImmediately")
    lock: Optional[Link] = Field(default=None, description="Restrict the user from logging in and performing any actions.  # Conditions  - User is not locked - `admin`")
    unlock: Optional[Link] = Field(default=None, description="Allow a locked user to login and act again.  # Conditions  - User is not locked - `admin`")
    delete: Optional[Link] = Field(default=None, description="Permanently remove a user from the instance  # Conditions  either:   - `admin`   - Setting `users_deletable_by_admin` is set or:   - User is self   - Setting `users_deletable_by_self` is set")
    auth_source: Optional[Link] = Field(default=None, description="Permanently remove a user from the instance  # Conditions  - LDAP authentication configured - `admin`", alias="authSource")
    __properties: ClassVar[List[str]] = ["showUser", "updateImmediately", "lock", "unlock", "delete", "authSource"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of show_user
        if self.show_user:
            _dict['showUser'] = self.show_user.to_dict()
        ...
        # override the default output from pydantic by calling `to_dict()` of auth_source
        if self.auth_source:
            _dict['authSource'] = self.auth_source.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "showUser": Link.from_dict(obj["showUser"]) if obj.get("showUser") is not None else None,
            "updateImmediately": Link.from_dict(obj["updateImmediately"]) if obj.get("updateImmediately") is not None else None,
            "lock": Link.from_dict(obj["lock"]) if obj.get("lock") is not None else None,
            "unlock": Link.from_dict(obj["unlock"]) if obj.get("unlock") is not None else None,
            "delete": Link.from_dict(obj["delete"]) if obj.get("delete") is not None else None,
            "authSource": Link.from_dict(obj["authSource"]) if obj.get("authSource") is not None else None
        })
        return _obj
