...
class WorkPackageFormModelLinks(BaseModel):
    """
    WorkPackageFormModelLinks
    """ # noqa: E501
    var_self: Optional[Link] = Field(default=None, description="This form endpoint  **Resource** : Form", alias="self")
    validate: Optional[Link] = Field(default=None, description="The endpoint for validating the request bodies. Often referring to this very form endpoint.")
    preview_markup: Optional[Link] = Field(default=None, description="Renders a markup preview for the work package form.", alias="previewMarkup")
    custom_fields: Optional[Link] = Field(default=None, description="Link to the HTML page for the custom field definitions.", alias="customFields")
    configure_form: Optional[Link] = Field(default=None, description="Link to the HTML page for the form configuration.", alias="configureForm")
    __properties: ClassVar[List[str]] = ["self", "validate", "previewMarkup", "customFields", "configureForm"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of configure_form
        if self.configure_form:
            _dict['configureForm'] = self.configure_form.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "validate": Link.from_dict(obj["validate"]) if obj.get("validate") is not None else None,
            "previewMarkup": Link.from_dict(obj["previewMarkup"]) if obj.get("previewMarkup") is not None else None,
            "customFields": Link.from_dict(obj["customFields"]) if obj.get("customFields") is not None else None,
            "configureForm": Link.from_dict(obj["configureForm"]) if obj.get("configureForm") is not None else None
        })
        return _obj
