...
class Formattable(BaseModel):
    ...
    Formattable
    """ # noqa: E501
    format: StrictStr = Field(description="Indicates the formatting language of the raw text")
    raw: Optional[StrictStr] = Field(default=None, description="The raw text, as entered by the user")
    html: Optional[StrictStr] = Field(default=None, description="The text converted to HTML according to the format")
    __properties: ClassVar[List[str]] = ["format", "raw", "html"]

    @field_validator('format')
    def format_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['plain', 'markdown', 'custom']):
            raise ValueError("must be one of enum values ('plain', 'markdown', 'custom')")
        return value
