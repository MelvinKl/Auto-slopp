...
class CustomActionModelLinks(BaseModel):
    """
    CustomActionModelLinks
    """ # noqa: E501
    var_self: Link = Field(description="This custom action  **Resource**: CustomAction", alias="self")
    execute_immediately: Link = Field(description="Execute this custom action.", alias="executeImmediately")
    __properties: ClassVar[List[str]] = ["self", "executeImmediately"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of execute_immediately
        if self.execute_immediately:
            _dict['executeImmediately'] = self.execute_immediately.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "executeImmediately": Link.from_dict(obj["executeImmediately"]) if obj.get("executeImmediately") is not None else None
        })
        return _obj
