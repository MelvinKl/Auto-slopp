...
class PortfolioCollectionModelAllOfLinks(BaseModel):
    """
    PortfolioCollectionModelAllOfLinks
    """ # noqa: E501
    var_self: Link = Field(description="This collection resource.  **Resource**: Collection", alias="self")
    jump_to: Link = Field(description="A templated link to jump to a given offset.", alias="jumpTo")
    change_size: Link = Field(description="A templated link to change the current page size.", alias="changeSize")
    previous_by_offset: Optional[Link] = Field(default=None, description="A link to the previous page of the collection.  # Conditions - The collection is not on the first page.", alias="previousByOffset")
    next_by_offset: Optional[Link] = Field(default=None, description="A link to the next page of the collection.  # Conditions - The collection is not on the last page.", alias="nextByOffset")
    representations: Optional[List[Link]] = None
    __properties: ClassVar[List[str]] = ["self", "jumpTo", "changeSize", "previousByOffset", "nextByOffset", "representations"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of jump_to
        if self.jump_to:
            _dict['jumpTo'] = self.jump_to.to_dict()
        # override the default output from pydantic by calling `to_dict()` of change_size
        if self.change_size:
            _dict['changeSize'] = self.change_size.to_dict()
        # override the default output from pydantic by calling `to_dict()` of previous_by_offset
        if self.previous_by_offset:
            _dict['previousByOffset'] = self.previous_by_offset.to_dict()
        # override the default output from pydantic by calling `to_dict()` of next_by_offset
        if self.next_by_offset:
            _dict['nextByOffset'] = self.next_by_offset.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "jumpTo": Link.from_dict(obj["jumpTo"]) if obj.get("jumpTo") is not None else None,
            "changeSize": Link.from_dict(obj["changeSize"]) if obj.get("changeSize") is not None else None,
            "previousByOffset": Link.from_dict(obj["previousByOffset"]) if obj.get("previousByOffset") is not None else None,
            "nextByOffset": Link.from_dict(obj["nextByOffset"]) if obj.get("nextByOffset") is not None else None,
            "representations": [Link.from_dict(_item) for _item in obj["representations"]] if obj.get("representations") is not None else None
        })
        return _obj
