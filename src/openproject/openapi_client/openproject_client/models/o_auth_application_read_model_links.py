...
class OAuthApplicationReadModelLinks(BaseModel):
    """
    OAuthApplicationReadModelLinks
    """ # noqa: E501
    var_self: Link = Field(description="This OAuth application  **Resource**: OAuthApplication", alias="self")
    owner: Link = Field(description="The user that created the OAuth application.  **Resource**: User")
    integration: Optional[Link] = Field(default=None, description="The resource that integrates this OAuth application into itself. Currently, only `Storage` resources are able to create and maintain own OAuth application.  **Resource**: Storage")
    redirect_uri: List[Link] = Field(alias="redirectUri")
    __properties: ClassVar[List[str]] = ["self", "owner", "integration", "redirectUri"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in redirect_uri (list)
        _items = []
        if self.redirect_uri:
            for _item_redirect_uri in self.redirect_uri:
                if _item_redirect_uri:
                    _items.append(_item_redirect_uri.to_dict())
            _dict['redirectUri'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "owner": Link.from_dict(obj["owner"]) if obj.get("owner") is not None else None,
            "integration": Link.from_dict(obj["integration"]) if obj.get("integration") is not None else None,
            "redirectUri": [Link.from_dict(_item) for _item in obj["redirectUri"]] if obj.get("redirectUri") is not None else None
        })
        return _obj
