...
class NotificationModelLinks(BaseModel):
    """
    NotificationModelLinks
    """ # noqa: E501
    var_self: Link = Field(description="This notification  **Resource**: Notification", alias="self")
    read_ian: Optional[Link] = Field(default=None, description="Request to mark the notification as read. Only available if the notification is currently unread.", alias="readIAN")
    unread_ian: Optional[Link] = Field(default=None, description="Request to mark the notification as unread. Only available if the notification is currently read.", alias="unreadIAN")
    project: Link = Field(description="The workspace the notification originated in  **Resource**: Workspace")
    actor: Link = Field(description="The user that caused the notification. This might be null in case no user triggered the notification.  **Resource**: User")
    resource: Link = Field(description="The linked resource of the notification, if any.  **Resource**: Polymorphic")
    activity: Link = Field(description="The journal activity, if the notification originated from a journal entry. This might be null in case no activity triggered the notification.  **Resource**: Activity")
    __properties: ClassVar[List[str]] = ["self", "readIAN", "unreadIAN", "project", "actor", "resource", "activity"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of read_ian
        if self.read_ian:
            _dict['readIAN'] = self.read_ian.to_dict()
        # override the default output from pydantic by calling `to_dict()` of unread_ian
        if self.unread_ian:
            _dict['unreadIAN'] = self.unread_ian.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "readIAN": Link.from_dict(obj["readIAN"]) if obj.get("readIAN") is not None else None,
            "unreadIAN": Link.from_dict(obj["unreadIAN"]) if obj.get("unreadIAN") is not None else None,
            "project": Link.from_dict(obj["project"]) if obj.get("project") is not None else None,
            "actor": Link.from_dict(obj["actor"]) if obj.get("actor") is not None else None,
            "resource": Link.from_dict(obj["resource"]) if obj.get("resource") is not None else None,
