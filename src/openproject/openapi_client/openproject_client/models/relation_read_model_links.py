...
class RelationReadModelLinks(BaseModel):
    """
    RelationReadModelLinks
    """ # noqa: E501
    var_self: Optional[Link] = Field(default=None, description="This relation  **Resource**: Relation  # Conditions  **Permission**: view work packages", alias="self")
    update_immediately: Optional[Link] = Field(default=None, description="Updates the relation between two work packages  # Conditions  **Permission**: manage work package relations", alias="updateImmediately")
    delete: Optional[Link] = Field(default=None, description="Destroys the relation between the two work packages  # Conditions  **Permission**: manage work package relations")
    var_from: Optional[Link] = Field(default=None, description="The emanating work package  **Resource**: WorkPackage  # Conditions  **Permission**: view work packages", alias="from")
    to: Optional[Link] = Field(default=None, description="The work package the relation ends in  **Resource**: WorkPackage  # Conditions  **Permission**: view work packages")
    __properties: ClassVar[List[str]] = ["self", "updateImmediately", "delete", "from", "to"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "from": Link.from_dict(obj["from"]) if obj.get("from") is not None else None,
            "to": Link.from_dict(obj["to"]) if obj.get("to") is not None else None
        })
        return _obj
