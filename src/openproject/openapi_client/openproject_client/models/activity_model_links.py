...
class ActivityModelLinks(BaseModel):
    """
    ActivityModelLinks
    """ # noqa: E501
    var_self: Optional[Link] = Field(default=None, description="This activity  **Resource**: Activity", alias="self")
    work_package: Optional[Link] = Field(default=None, description="The work package this activity belongs to  **Resource**: WorkPackage", alias="workPackage")
    user: Optional[Link] = Field(default=None, description="The user who created this activity  **Resource**: Principal")
    update: Optional[Link] = Field(default=None, description="Update this activity")
    attachments: Optional[Link] = Field(default=None, description="The attachment collection of this activity  **Resource**: Attachments")
    add_attachment: Optional[Link] = Field(default=None, description="Attach a file to the activity  # Conditions  **Permissions**: - `add_work_package_comments` - for internal comments: `add_internal_comments`", alias="addAttachment")
    emoji_reactions: Optional[Link] = Field(default=None, description="The emoji reactions collection of this activity  **Resource**: EmojiReactions", alias="emojiReactions")
    __properties: ClassVar[List[str]] = ["self", "workPackage", "user", "update", "attachments", "addAttachment", "emojiReactions"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of work_package
        if self.work_package:
            _dict['workPackage'] = self.work_package.to_dict()
        ...
        # override the default output from pydantic by calling `to_dict()` of emoji_reactions
        if self.emoji_reactions:
            _dict['emojiReactions'] = self.emoji_reactions.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "workPackage": Link.from_dict(obj["workPackage"]) if obj.get("workPackage") is not None else None,
            ...
            "emojiReactions": Link.from_dict(obj["emojiReactions"]) if obj.get("emojiReactions") is not None else None
        })
        return _obj
