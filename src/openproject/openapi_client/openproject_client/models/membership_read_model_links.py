...
class MembershipReadModelLinks(BaseModel):
    """
    MembershipReadModelLinks
    """ # noqa: E501
    var_self: Link = Field(description="This membership.  **Resource**: Membership", alias="self")
    var_schema: Link = Field(description="This membership schema.  **Resource**: Schema", alias="schema")
    update: Optional[Link] = Field(default=None, description="The endpoint for updating the membership.  # Conditions  **Permission**: manage_members")
    update_immediately: Optional[Link] = Field(default=None, description="The endpoint for updating the membership without form validation.  # Conditions  **Permission**: manage_members", alias="updateImmediately")
    project: Link = Field(description="The workspace the membership is related to.  **Resource**: Workspace")
    principal: Link = Field(description="The principal the membership is related to.  **Resource**: Principal")
    roles: List[Link]
    __properties: ClassVar[List[str]] = ["self", "schema", "update", "updateImmediately", "project", "principal", "roles"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "principal": Link.from_dict(obj["principal"]) if obj.get("principal") is not None else None,
            "roles": [Link.from_dict(_item) for _item in obj["roles"]] if obj.get("roles") is not None else None
        })
        return _obj
