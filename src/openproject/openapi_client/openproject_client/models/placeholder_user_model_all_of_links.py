...
class PlaceholderUserModelAllOfLinks(BaseModel):
    """
    PlaceholderUserModelAllOfLinks
    """ # noqa: E501
    show_user: Link = Field(description="A relative path to show the placeholder user in the web application.", alias="showUser")
    delete: Optional[Link] = Field(default=None, description="An href to delete the placeholder user.  # Conditions:  - `manage_placeholder_user`")
    update_immediately: Optional[Link] = Field(default=None, description="An href to update the placeholder user.  # Conditions:  - `manage_placeholder_user`  **Resource**: PlaceholderUser", alias="updateImmediately")
    __properties: ClassVar[List[str]] = ["showUser", "delete", "updateImmediately"]
