...
class PlaceholderUserModel(BaseModel):
    """
    ...
    updated_at: Optional[datetime] = Field(default=None, description="Time of the most recent change to the principal", alias="updatedAt")
    links: PlaceholderUserModelAllOfLinks = Field(alias="_links")
    status: Optional[StrictStr] = Field(default=None, description="The current activation status of the placeholder user.  # Conditions  - User has `manage_placeholder_user` permission globally")
    __properties: ClassVar[List[str]] = ["_type", "id", "name", "createdAt", "updatedAt", "_links", "status"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['PlaceholderUser']):
            raise ValueError("must be one of enum values ('PlaceholderUser')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "_links": PlaceholderUserModelAllOfLinks.from_dict(obj["_links"]) if obj.get("_links") is not None else None,
