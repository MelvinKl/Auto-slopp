...
class MembershipFormModelLinks(BaseModel):
    """
    MembershipFormModelLinks
    """ # noqa: E501
    var_self: Link = Field(description="This form request.  **Resource**: Form", alias="self")
    validate: List[Link]
    commit: Link = Field(description="The endpoint to create the membership with the same payload, as sent to the form.  **Resource**: Membership")
    __properties: ClassVar[List[str]] = ["self", "validate", "commit"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in validate (list)
        _items = []
        if self.validate:
            for _item_validate in self.validate:
                if _item_validate:
                    _items.append(_item_validate.to_dict())
            _dict['validate'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "validate": [Link.from_dict(_item) for _item in obj["validate"]] if obj.get("validate") is not None else None,
            "commit": Link.from_dict(obj["commit"]) if obj.get("commit") is not None else None
        })
        return _obj
