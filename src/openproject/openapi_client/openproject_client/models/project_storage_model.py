...
class ProjectStorageModel(BaseModel):
    """
    ProjectStorageModel
    """ # noqa: E501
    type: StrictStr = Field(alias="_type")
    id: Annotated[int, Field(strict=True, ge=1)] = Field(description="The project storage's id")
    project_folder_mode: StrictStr = Field(alias="projectFolderMode")
    created_at: datetime = Field(description="Time of creation", alias="createdAt")
    updated_at: datetime = Field(description="Time of the most recent change to the project storage", alias="updatedAt")
    links: Optional[ProjectStorageModelLinks] = Field(default=None, alias="_links")
    __properties: ClassVar[List[str]] = ["_type", "id", "projectFolderMode", "createdAt", "updatedAt", "_links"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ProjectStorage']):
            raise ValueError("must be one of enum values ('ProjectStorage')")
        return value

    @field_validator('project_folder_mode')
    def project_folder_mode_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['inactive', 'manual']):
            raise ValueError("must be one of enum values ('inactive', 'manual')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "_type": obj.get("_type"),
            "id": obj.get("id"),
            "projectFolderMode": obj.get("projectFolderMode"),
            "createdAt": obj.get("createdAt"),
            "updatedAt": obj.get("updatedAt"),
            "_links": ProjectStorageModelLinks.from_dict(obj["_links"]) if obj.get("_links") is not None else None
        })
        return _obj
