...
class GridWidgetModel(BaseModel):
    """
    GridWidgetModel
    """ # noqa: E501
    type: StrictStr = Field(alias="_type")
    id: Optional[Annotated[int, Field(strict=True, ge=1)]] = Field(description="The grid widget's unique identifier. Can be null, if a new widget is created within a grid.")
    identifier: StrictStr = Field(description="An alternative, human legible, and unique identifier.")
    start_row: Annotated[int, Field(strict=True, ge=1)] = Field(description="The index of the starting row of the widget. The row is inclusive.", alias="startRow")
    end_row: Annotated[int, Field(strict=True, ge=1)] = Field(description="The index of the ending row of the widget. The row is exclusive.", alias="endRow")
    start_column: Annotated[int, Field(strict=True, ge=1)] = Field(description="The index of the starting column of the widget. The column is inclusive.", alias="startColumn")
    end_column: Annotated[int, Field(strict=True, ge=1)] = Field(description="The index of the ending column of the widget. The column is exclusive.", alias="endColumn")
    options: Optional[Dict[str, Any]] = None
    __properties: ClassVar[List[str]] = ["_type", "id", "identifier", "startRow", "endRow", "startColumn", "endColumn", "options"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['GridWidget']):
            raise ValueError("must be one of enum values ('GridWidget')")
        return value
