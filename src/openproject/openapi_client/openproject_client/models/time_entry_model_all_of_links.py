...
class TimeEntryModelAllOfLinks(BaseModel):
    """
    TimeEntryModelAllOfLinks
    """ # noqa: E501
    var_self: Link = Field(description="This time entry  **Resource**: TimeEntry", alias="self")
    update_immediately: Optional[Link] = Field(default=None, description="Directly perform edits on this time entry  # Conditions  **Permission**: 'edit time entries' or 'edit own time entries' if the time entry belongs to the user", alias="updateImmediately")
    update: Optional[Link] = Field(default=None, description="Form endpoint that aids in preparing and performing edits on a TimeEntry  # Conditions  **Permission**: 'edit time entries' or 'edit own time entries' if the time entry belongs to the user")
    delete: Optional[Link] = Field(default=None, description="Delete this time entry  # Conditions  **Permission**: 'edit time entries' or 'edit own time entries' if the time entry belongs to the user")
    var_schema: Optional[Link] = Field(default=None, description="The time entry schema  **Resource**: Schema", alias="schema")
    project: Link = Field(description="The project the time entry is bundled in. The project might be different from the work package's project once the workPackage is moved.  **Resource**: Project")
    entity: Link = Field(description="The entity the time entry is created on  **Resource**: WorkPackage, Meeting")
    user: Link = Field(description="The user the time entry tracks expenditures for  **Resource**: User")
    activity: Link = Field(description="The time entry activity the time entry is categorized as  **Resource**: TimeEntriesActivity")
    __properties: ClassVar[List[str]] = ["self", "updateImmediately", "update", "delete", "schema", "project", "entity", "user", "activity"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "entity": Link.from_dict(obj["entity"]) if obj.get("entity") is not None else None,
            "user": Link.from_dict(obj["user"]) if obj.get("user") is not None else None,
            "activity": Link.from_dict(obj["activity"]) if obj.get("activity") is not None else None
        })
        return _obj
