...
class StorageFileUploadLinkModelLinks(BaseModel):
    """
    StorageFileUploadLinkModelLinks
    """ # noqa: E501
    var_self: Link = Field(description="The resource link of the upload link resource.  As the upload link is a temporal object, and cannot be retrieved again, the self link usually is `urn:openproject-org:api:v3:storages:upload_link:no_link_provided`.  **Resource**: UploadLink", alias="self")
    destination: Link = Field(description="The direct upload link.  **Resource**: N/A")
    __properties: ClassVar[List[str]] = ["self", "destination"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "destination": Link.from_dict(obj["destination"]) if obj.get("destination") is not None else None
        })
        return _obj
