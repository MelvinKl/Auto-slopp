...
class NewsModelLinks(BaseModel):
    """
    NewsModelLinks
    """ # noqa: E501
    var_self: Link = Field(description="This news  **Resource**: News", alias="self")
    project: Link = Field(description="The workspace the news is situated in  **Resource**: workspace")
    author: Link = Field(description="The user having created the news  **Resource**: User")
    update_immediately: Optional[Link] = Field(default=None, description="Directly perform edits on the news  **Permission** manage news", alias="updateImmediately")
    delete: Optional[Link] = Field(default=None, description="Delete the news  **Permission** manage news")
    __properties: ClassVar[List[str]] = ["self", "project", "author", "updateImmediately", "delete"]
