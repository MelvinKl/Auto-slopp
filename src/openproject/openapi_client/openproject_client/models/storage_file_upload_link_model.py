...
class StorageFileUploadLinkModel(BaseModel):
    """
    StorageFileUploadLinkModel
    """ # noqa: E501
    type: StrictStr = Field(alias="_type")
    links: StorageFileUploadLinkModelLinks = Field(alias="_links")
    __properties: ClassVar[List[str]] = ["_type", "_links"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['UploadLink']):
            raise ValueError("must be one of enum values ('UploadLink')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "_type": obj.get("_type"),
            "_links": StorageFileUploadLinkModelLinks.from_dict(obj["_links"]) if obj.get("_links") is not None else None
        })
        return _obj
