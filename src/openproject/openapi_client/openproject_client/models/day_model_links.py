...
class DayModelLinks(BaseModel):
    """
    DayModelLinks
    """ # noqa: E501
    var_self: Link = Field(alias="self")
    non_working_reasons: Optional[List[Link]] = Field(default=None, description="A list of resources describing why this day is a non-working day. Linked resources can be `NonWorkingDay` and `WeekDay` resources. This property is absent for working days.", alias="nonWorkingReasons")
    week_day: Optional[Link] = Field(default=None, description="The week day for this day.", alias="weekDay")
    __properties: ClassVar[List[str]] = ["self", "nonWorkingReasons", "weekDay"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in non_working_reasons (list)
        _items = []
        if self.non_working_reasons:
            for _item_non_working_reasons in self.non_working_reasons:
                if _item_non_working_reasons:
                    _items.append(_item_non_working_reasons.to_dict())
            _dict['nonWorkingReasons'] = _items
        # override the default output from pydantic by calling `to_dict()` of week_day
        if self.week_day:
            _dict['weekDay'] = self.week_day.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "nonWorkingReasons": [Link.from_dict(_item) for _item in obj["nonWorkingReasons"]] if obj.get("nonWorkingReasons") is not None else None,
            "weekDay": Link.from_dict(obj["weekDay"]) if obj.get("weekDay") is not None else None
        })
        return _obj
