...
class RevisionModelLinks(BaseModel):
    """
    RevisionModelLinks
    """ # noqa: E501
    var_self: Link = Field(description="This revision  **Resource**: Revision", alias="self")
    project: Link = Field(description="The project to which the revision belongs  **Resource**: Project")
    author: Optional[Link] = Field(default=None, description="The user that added this revision, if the authorName was mapped to a user in OpenProject  **Resource**: User")
    show_revision: Link = Field(description="A URL to the repository view (outside APIv3) showing this revision  **Resource**: -", alias="showRevision")
    __properties: ClassVar[List[str]] = ["self", "project", "author", "showRevision"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * OpenAPI `readOnly` fields are excluded.
        ...
        # override the default output from pydantic by calling `to_dict()` of show_revision
        if self.show_revision:
            _dict['showRevision'] = self.show_revision.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "showRevision": Link.from_dict(obj["showRevision"]) if obj.get("showRevision") is not None else None
        })
        return _obj
