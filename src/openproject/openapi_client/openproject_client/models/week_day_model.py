...
class WeekDayModel(BaseModel):
    ...
    day: Annotated[int, Field(le=7, strict=True, ge=1)] = Field(description="The week day from 1 to 7. 1 is Monday. 7 is Sunday.")
    name: StrictStr = Field(description="The week day name.")
    working: StrictBool = Field(description="`true` for a working week day, `false` otherwise.")
    links: Optional[WeekDaySelfLinkModel] = Field(default=None, alias="_links")
    __properties: ClassVar[List[str]] = ["_type", "day", "name", "working", "_links"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['WeekDay']):
            raise ValueError("must be one of enum values ('WeekDay')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "_links": WeekDaySelfLinkModel.from_dict(obj["_links"]) if obj.get("_links") is not None else None
        })
        return _obj
