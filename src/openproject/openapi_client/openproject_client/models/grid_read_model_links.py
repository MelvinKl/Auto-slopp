...
class GridReadModelLinks(BaseModel):
    """
    GridReadModelLinks
    """ # noqa: E501
    var_self: Link = Field(description="This grid.  **Resource**: Grid", alias="self")
    attachments: Optional[Link] = Field(default=None, description="The attachment collection of this grid.  **Resource**: AttachmentCollection")
    add_attachment: Optional[Link] = Field(default=None, description="Link for adding an attachment to this grid.", alias="addAttachment")
    scope: Link = Field(description="The location where this grid is used, usually represented as a relative URL.")
    update_immediately: Optional[Link] = Field(default=None, description="Directly perform edits on this grid.  # Conditions  **Permission**: depends on the page the grid is defined for", alias="updateImmediately")
    update: Optional[Link] = Field(default=None, description="Validate edits on the grid via a form resource before committing  # Conditions  **Permission**: depends on the page the grid is defined for")
    delete: Optional[Link] = Field(default=None, description="Deletes this grid resource.")
    __properties: ClassVar[List[str]] = ["self", "attachments", "addAttachment", "scope", "updateImmediately", "update", "delete"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "scope": Link.from_dict(obj["scope"]) if obj.get("scope") is not None else None,
