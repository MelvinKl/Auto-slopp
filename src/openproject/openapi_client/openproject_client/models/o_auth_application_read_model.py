...
class OAuthApplicationReadModel(BaseModel):
    """
    OAuthApplicationReadModel
    """ # noqa: E501
    id: Annotated[int, Field(strict=True, ge=1)]
    type: StrictStr = Field(alias="_type")
    name: StrictStr = Field(description="The name of the OAuth 2 application")
    client_id: StrictStr = Field(description="OAuth 2 client id", alias="clientId")
    client_secret: Optional[StrictStr] = Field(default=None, description="OAuth 2 client secret. This is only returned when creating a new OAuth application.", alias="clientSecret")
    confidential: StrictBool = Field(description="true, if OAuth 2 credentials are confidential, false, if no secret is stored")
    created_at: Optional[datetime] = Field(default=None, description="The time the OAuth 2 Application was created at", alias="createdAt")
    updated_at: Optional[datetime] = Field(default=None, description="The time the OAuth 2 Application was last updated", alias="updatedAt")
    scopes: Optional[List[StrictStr]] = Field(default=None, description="An array of the scopes of the OAuth 2 Application")
    links: OAuthApplicationReadModelLinks = Field(alias="_links")
    __properties: ClassVar[List[str]] = ["id", "_type", "name", "clientId", "clientSecret", "confidential", "createdAt", "updatedAt", "scopes", "_links"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['OAuthApplication']):
            raise ValueError("must be one of enum values ('OAuthApplication')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "_links": OAuthApplicationReadModelLinks.from_dict(obj["_links"]) if obj.get("_links") is not None else None
        })
        return _obj
