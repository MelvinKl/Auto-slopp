...
class PrincipalModelLinks(BaseModel):
    """
    PrincipalModelLinks
    """ # noqa: E501
    var_self: Link = Field(description="This principal resource.  **Resource**: User|Group|PlaceholderUser", alias="self")
    memberships: Optional[Link] = Field(default=None, description="An href to the collection of the principal's memberships.  # Conditions:  - user has permission `view_members` or `manage_members` in any project  **Resource**: Collection")
    __properties: ClassVar[List[str]] = ["self", "memberships"]
