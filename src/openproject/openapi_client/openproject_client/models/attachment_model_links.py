...
class AttachmentModelLinks(BaseModel):
    """
    AttachmentModelLinks
    """ # noqa: E501
    delete: Optional[Link] = Field(default=None, description="Deletes this attachment  # Conditions  **Permission**: edit on attachment container or being the author for attachments without container")
    var_self: Link = Field(description="This attachment  **Resource**: Attachment", alias="self")
    container: Link = Field(description="The object (e.g. WorkPackage) housing the attachment  **Resource**: Anything")
    author: Link = Field(description="The user who uploaded the attachment  **Resource**: User")
    download_location: Link = Field(description="Direct download link to the attachment  **Resource**: -", alias="downloadLocation")
    __properties: ClassVar[List[str]] = ["delete", "self", "container", "author", "downloadLocation"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of download_location
        if self.download_location:
            _dict['downloadLocation'] = self.download_location.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "downloadLocation": Link.from_dict(obj["downloadLocation"]) if obj.get("downloadLocation") is not None else None
        })
        return _obj
