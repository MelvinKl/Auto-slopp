...
class ProjectStorageModelLinks(BaseModel):
    """
    ProjectStorageModelLinks
    """ # noqa: E501
    var_self: Link = Field(description="This project storage.  **Resource**: ProjectStorage", alias="self")
    creator: Link = Field(description="The user who created the project storage.  **Resource**: User")
    storage: Link = Field(description="The storage resource, that is linked to a project with this project storage.  **Resource**: Storage")
    project: Link = Field(description="The project resource, that is linked to a storage with this project storage.  **Resource**: Project")
    project_folder: Optional[Link] = Field(default=None, description="The directory on the storage that is used as a project folder.  **Resource**: StorageFile  # Conditions  Only provided, if the `projectFolderMode` is `manual` or `automatic`.", alias="projectFolder")
    open: Optional[Link] = Field(default=None, description="A link to OpenProject strorage.  # Conditions  If the storage has not been configured(oauth client is missing, for instance), then the link is null.")
    open_with_connection_ensured: Optional[Link] = Field(default=None, description="A link to OpenProject storage with making sure user has access to it.  **Deprecated:** Use `open` instead, which returns a link that will ensure the user's connection to the storage as well, but properly works for all kinds of storage configurations.", alias="openWithConnectionEnsured")
    __properties: ClassVar[List[str]] = ["self", "creator", "storage", "project", "projectFolder", "open", "openWithConnectionEnsured"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of project_folder
        if self.project_folder:
            _dict['projectFolder'] = self.project_folder.to_dict()
        # override the default output from pydantic by calling `to_dict()` of open
        if self.open:
            _dict['open'] = self.open.to_dict()
        # override the default output from pydantic by calling `to_dict()` of open_with_connection_ensured
        if self.open_with_connection_ensured:
            _dict['openWithConnectionEnsured'] = self.open_with_connection_ensured.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "projectFolder": Link.from_dict(obj["projectFolder"]) if obj.get("projectFolder") is not None else None,
            "open": Link.from_dict(obj["open"]) if obj.get("open") is not None else None,
            "openWithConnectionEnsured": Link.from_dict(obj["openWithConnectionEnsured"]) if obj.get("openWithConnectionEnsured") is not None else None
        })
        return _obj
