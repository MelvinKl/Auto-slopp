...
class MembershipWriteModelLinks(BaseModel):
    """
    MembershipWriteModelLinks
    """ # noqa: E501
    principal: Optional[Link] = Field(default=None, description="The principal that is to get a membership.  **Resource**: Principal")
    roles: Optional[List[Link]] = None
    project: Optional[Link] = Field(default=None, description="The project that is to get a membership. If no project is given, the principal's membership is supposed to be global.  **Resource**: Project")
    __properties: ClassVar[List[str]] = ["principal", "roles", "project"]
