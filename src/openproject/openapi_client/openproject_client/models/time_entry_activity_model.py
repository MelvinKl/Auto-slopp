...
class TimeEntryActivityModel(BaseModel):
    """
    TimeEntryActivityModel
    """ # noqa: E501
    type: StrictStr = Field(alias="_type")
    id: Annotated[int, Field(strict=True, ge=1)] = Field(description="Time entry id")
    name: StrictStr = Field(description="The human readable name chosen for this activity")
    position: StrictInt = Field(description="The rank the activity has in a list of activities")
    default: StrictBool = Field(description="Flag to signal whether this activity is the default activity")
    embedded: TimeEntryActivityModelEmbedded = Field(alias="_embedded")
    links: TimeEntryActivityModelLinks = Field(alias="_links")
    __properties: ClassVar[List[str]] = ["_type", "id", "name", "position", "default", "_embedded", "_links"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['TimeEntriesActivity']):
            raise ValueError("must be one of enum values ('TimeEntriesActivity')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "_embedded": TimeEntryActivityModelEmbedded.from_dict(obj["_embedded"]) if obj.get("_embedded") is not None else None,
            "_links": TimeEntryActivityModelLinks.from_dict(obj["_links"]) if obj.get("_links") is not None else None
        })
        return _obj
