...
class VersionReadModelAllOfLinks(BaseModel):
    """
    VersionReadModelAllOfLinks
    """ # noqa: E501
    var_self: Link = Field(description="This version  **Resource**: Version", alias="self")
    var_schema: Link = Field(description="The schema of this version  **Resource**: VersionSchema", alias="schema")
    update: Optional[Link] = Field(default=None, description="Form endpoint that aids in preparing and performing edits on the version  # Conditions  **Permission**: manage versions")
    delete: Optional[Link] = Field(default=None, description="Deletes this version  # Conditions  **Permission**: manage versions")
    update_immediately: Optional[Link] = Field(default=None, description="Directly perform edits on the version  # Conditions  **Permission**: manage versions", alias="updateImmediately")
    defining_project: Link = Field(description="The workspace to which the version belongs  **Resource**: Workspace", alias="definingProject")
    available_in_projects: Link = Field(description="Workspaces where this version can be used  **Resource**: Workspace", alias="availableInProjects")
    __properties: ClassVar[List[str]] = ["self", "schema", "update", "delete", "updateImmediately", "definingProject", "availableInProjects"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of defining_project
        if self.defining_project:
            _dict['definingProject'] = self.defining_project.to_dict()
        # override the default output from pydantic by calling `to_dict()` of available_in_projects
        if self.available_in_projects:
            _dict['availableInProjects'] = self.available_in_projects.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "definingProject": Link.from_dict(obj["definingProject"]) if obj.get("definingProject") is not None else None,
            "availableInProjects": Link.from_dict(obj["availableInProjects"]) if obj.get("availableInProjects") is not None else None
        })
        return _obj
