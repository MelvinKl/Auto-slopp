...
class WatchersModelAllOfEmbeddedElements(BaseModel):
    """
    Collection of Users
    """ # noqa: E501
    type: StrictStr = Field(alias="_type")
    id: Annotated[int, Field(strict=True, ge=1)] = Field(description="The principal's unique identifier.")
    name: StrictStr = Field(description="The principal's display name, layout depends on instance settings.")
    created_at: Optional[datetime] = Field(default=None, description="Time of creation", alias="createdAt")
    updated_at: Optional[datetime] = Field(default=None, description="Time of the most recent change to the user", alias="updatedAt")
    links: UserModelAllOfLinks = Field(alias="_links")
    avatar: StrictStr = Field(description="URL to user's avatar")
    login: Optional[Annotated[str, Field(strict=True, max_length=256)]] = Field(default=None, description="The user's login name  # Conditions  - User is self, or `create_user` or `manage_user` permission globally")
    first_name: Optional[Annotated[str, Field(strict=True, max_length=30)]] = Field(default=None, description="The user's first name  # Conditions  - User is self, or `create_user` or `manage_user` permission globally", alias="firstName")
    last_name: Optional[Annotated[str, Field(strict=True, max_length=30)]] = Field(default=None, description="The user's last name  # Conditions  - User is self, or `create_user` or `manage_user` permission globally", alias="lastName")
    email: Optional[Annotated[str, Field(strict=True, max_length=60)]] = Field(default=None, description="The user's email address  # Conditions  - E-Mail address not hidden - User is not a new record - User is self, or `create_user` or `manage_user` permission globally")
    admin: Optional[StrictBool] = Field(default=None, description="Flag indicating whether or not the user is an admin  # Conditions  - `admin`")
    status: Optional[StrictStr] = Field(default=None, description="The current activation status of the user.  # Conditions  - User is self, or `create_user` or `manage_user` permission globally")
    language: Optional[StrictStr] = Field(default=None, description="User's language | ISO 639-1 format  # Conditions  - User is self, or `create_user` or `manage_user` permission globally")
    identity_url: Optional[StrictStr] = Field(default=None, description="User's identity_url for OmniAuth authentication. **Deprecated:** It will be removed in the near future.  # Conditions  - User is self, or `create_user` or `manage_user` permission globally", alias="identityUrl")
    __properties: ClassVar[List[str]] = ["_type", "id", "name", "createdAt", "updatedAt", "_links", "avatar", "login", "firstName", "lastName", "email", "admin", "status", "language", "identityUrl"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of WatchersModelAllOfEmbeddedElements from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of WatchersModelAllOfEmbeddedElements from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "_links": UserModelAllOfLinks.from_dict(obj["_links"]) if obj.get("_links") is not None else None,
            ...
            "identityUrl": obj.get("identityUrl")
        })
        return _obj
