...
class RootModel(BaseModel):
    ...
    instance_name: StrictStr = Field(description="The name of the OpenProject instance", alias="instanceName")
    core_version: Optional[StrictStr] = Field(default=None, description="The OpenProject core version number for the instance  # Conditions  **Permission** requires admin privileges", alias="coreVersion")
    links: RootModelLinks = Field(alias="_links")
    __properties: ClassVar[List[str]] = ["_type", "instanceName", "coreVersion", "_links"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Root']):
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "coreVersion": obj.get("coreVersion"),
            "_links": RootModelLinks.from_dict(obj["_links"]) if obj.get("_links") is not None else None
        })
        return _obj
