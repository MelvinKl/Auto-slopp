...
class EmojiReactionModelLinks(BaseModel):
    """
    EmojiReactionModelLinks
    """ # noqa: E501
    var_self: Optional[Link] = Field(default=None, description="This emoji reaction", alias="self")
    reactable: Optional[Link] = Field(default=None, description="The activity this emoji reaction belongs to")
    reacting_users: Optional[List[Link]] = Field(default=None, alias="reactingUsers")
    __properties: ClassVar[List[str]] = ["self", "reactable", "reactingUsers"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of reactable
        if self.reactable:
            _dict['reactable'] = self.reactable.to_dict()
        # override the default output from pydantic by calling `to_dict()` of each item in reacting_users (list)
        _items = []
        if self.reacting_users:
            for _item_reacting_users in self.reacting_users:
                if _item_reacting_users:
                    _items.append(_item_reacting_users.to_dict())
            _dict['reactingUsers'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "reactable": Link.from_dict(obj["reactable"]) if obj.get("reactable") is not None else None,
            "reactingUsers": [Link.from_dict(_item) for _item in obj["reactingUsers"]] if obj.get("reactingUsers") is not None else None
        })
        return _obj
