...
class RootModelLinks(BaseModel):
    """
    RootModelLinks
    """ # noqa: E501
    var_self: Link = Field(description="This root information object.  **Resource**: Root", alias="self")
    configuration: Link = Field(description="The configuration resource.  **Resource**: Configuration")
    memberships: Link = Field(description="The collection of memberships.  **Resource**: Collection")
    priorities: Link = Field(description="The collection of priorities.  **Resource**: Collection")
    relations: Link = Field(description="The collection of relations.  **Resource**: Collection")
    statuses: Link = Field(description="The collection of statuses.  **Resource**: Collection")
    time_entries: Link = Field(description="The collection of time entries.  **Resource**: Collection")
    types: Link = Field(description="The collection of types.  **Resource**: Collection")
    user: Link = Field(description="The current user resource.  **Resource**: User")
    user_preferences: Link = Field(description="The current user preferences resource.  **Resource**: UserPreferences", alias="userPreferences")
    work_packages: Link = Field(description="The work package collection.  **Resource**: Collection", alias="workPackages")
    __properties: ClassVar[List[str]] = ["self", "configuration", "memberships", "priorities", "relations", "statuses", "time_entries", "types", "user", "userPreferences", "workPackages"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.time_entries:
            _dict['time_entries'] = self.time_entries.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "configuration": Link.from_dict(obj["configuration"]) if obj.get("configuration") is not None else None,
            "memberships": Link.from_dict(obj["memberships"]) if obj.get("memberships") is not None else None,
            "priorities": Link.from_dict(obj["priorities"]) if obj.get("priorities") is not None else None,
            ...
            "time_entries": Link.from_dict(obj["time_entries"]) if obj.get("time_entries") is not None else None,
            ...
            "userPreferences": Link.from_dict(obj["userPreferences"]) if obj.get("userPreferences") is not None else None,
