...
class OAuthClientCredentialsReadModel(BaseModel):
    """
    ...
    created_at: Optional[datetime] = Field(default=None, description="The time the OAuth client credentials were created at", alias="createdAt")
    updated_at: Optional[datetime] = Field(default=None, description="The time the OAuth client credentials were last updated", alias="updatedAt")
    links: OAuthClientCredentialsReadModelLinks = Field(alias="_links")
    __properties: ClassVar[List[str]] = ["id", "_type", "clientId", "confidential", "createdAt", "updatedAt", "_links"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['OAuthClientCredentials']):
            raise ValueError("must be one of enum values ('OAuthClientCredentials')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "_links": OAuthClientCredentialsReadModelLinks.from_dict(obj["_links"]) if obj.get("_links") is not None else None
        })
        return _obj
