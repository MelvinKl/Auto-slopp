...
class QueryFilterInstanceSchemaModelLinks(BaseModel):
    """
    QueryFilterInstanceSchemaModelLinks
    """ # noqa: E501
    var_self: Link = Field(description="This QueryFilterInstanceSchema (same as for schema)  **Resource**: QueryFilterInstanceSchema", alias="self")
    filter: Link = Field(description="The filter for which this schema is specific  **Resource**: Filter")
    __properties: ClassVar[List[str]] = ["self", "filter"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "filter": Link.from_dict(obj["filter"]) if obj.get("filter") is not None else None
        })
        return _obj
