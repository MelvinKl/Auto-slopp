...
class ValuesPropertyModel(BaseModel):
    """
    ValuesPropertyModel
    """ # noqa: E501
    type: StrictStr = Field(alias="_type")
    var_property: StrictStr = Field(description="The key of the key - value pair represented by the Values::Property", alias="property")
    value: StrictStr = Field(description="The value of the key - value pair represented by the Values::Property")
    links: ValuesPropertyModelLinks = Field(alias="_links")
    __properties: ClassVar[List[str]] = ["_type", "property", "value", "_links"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Values::Property']):
            raise ValueError("must be one of enum values ('Values::Property')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "value": obj.get("value"),
            "_links": ValuesPropertyModelLinks.from_dict(obj["_links"]) if obj.get("_links") is not None else None
        })
        return _obj
