...
class QueryModelLinks(BaseModel):
    """
    QueryModelLinks
    """ # noqa: E501
    star: Optional[Link] = Field(default=None, description="Elevates the query to the status of 'starred'  # Conditions  **Permission**: save queries for own queries, manage public queries for public queries; Only present if query is not yet starred")
    unstar: Optional[Link] = Field(default=None, description="Removes the 'starred' status  # Conditions  **Permission**: save queries for own queries, manage public queries for public queries; Only present if query is starred")
    update: Optional[Link] = Field(default=None, description="Use the Form based process to verify the query before persisting  # Conditions  **Permission**: view work packages")
    update_immediately: Optional[Link] = Field(default=None, description="Persist the query without using a Form based process for guidance  # Conditions  **Permission**: save queries for own queries, manage public queries for public queries;", alias="updateImmediately")
    __properties: ClassVar[List[str]] = ["star", "unstar", "update", "updateImmediately"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * OpenAPI `readOnly` fields are excluded.
        ...
        # override the default output from pydantic by calling `to_dict()` of star
        if self.star:
            _dict['star'] = self.star.to_dict()
        # override the default output from pydantic by calling `to_dict()` of unstar
        if self.unstar:
            _dict['unstar'] = self.unstar.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "star": Link.from_dict(obj["star"]) if obj.get("star") is not None else None,
            "unstar": Link.from_dict(obj["unstar"]) if obj.get("unstar") is not None else None,
